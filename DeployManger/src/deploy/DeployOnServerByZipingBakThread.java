...
@Slf4j
@Data
public class DeployOnServerByZipingBakThread implements Runnable {
    ...
    public void run() {
        ...
        getWasSourceDownByBaseSystem(props, deploySystemFtp, bacupYn);
        ...
        System.out.println("ThreadName =" + Thread.currentThread().getName());
        System.out.println("deploySystemFtp =" + deploySystemFtp);
        System.out.println("DeployOnServerByZipingThread WasSourceDownByBaseSystem END");
        System.out.println("==============================================");

    }


    public DeployOnServerByZipingBakThread(Properties props, String deploySystemFtp, boolean bacupYn) {

        this.props = props;
        this.deploySystemFtp = deploySystemFtp;
        this.bacupYn = bacupYn;
        propServerSet(props);
    }
    ...
    public static void getWasSourceDownByBaseSystem(Properties props, String basetSystem, boolean bacupYn) {
        ...
        if (bacupYn) {
            ...
            bakupFileControll(deployTempDir, basetSystem, zipping_shell_web_server, jspwork_zipping_dir);
        }
    ...
    private static void bakupFileControll(String deployTempDir, String basetSystem, String zipping_shell_web_server,
            String jspwork_zipping_dir) {

        String todate = DateUtils.getDate("yyyyMMdd");

        File bakDir = new File(deployTempDir + "/" + todate);

        // 오늘 날짜 dir생성
        if (!bakDir.exists()) {
            makeDiretory(deployTempDir + "/" + todate);
        }
        ...
        if (basetSystem.contains("IMAGE")) {
            command = "mv " + deployTempDir + zipping_shell_web_server + "_WEB.tar " + deployTempDir + todate + "/";
            commandExecCommon(command);
        } else {
            command = "mv " + deployTempDir + zipping_shell_web_server + "_WEB.tar " + deployTempDir + todate + "/";
            commandExecCommon(command);

            command = "mv " + deployTempDir + zipping_shell_web_server + "_WAS.tar " + deployTempDir + todate + "/";
            commandExecCommon(command);

            if (StringUtils.isNotEmpty(jspwork_zipping_dir)) {
                command = "mv " + deployTempDir + zipping_shell_web_server + "_jspwork.tar " + deployTempDir + todate + "/";
                commandExecCommon(command);
            }
        }
    ...
    private static void commandExecCommon(String command) {
    ...
    private static void delCompileLogFile(String deploySystem, SFTPService ftp) {
        ...
        if (StringUtils.isNotEmpty(deploySystem)) {
            String command = "rm log/javac_result_" + deploySystem + "_*.txt";
